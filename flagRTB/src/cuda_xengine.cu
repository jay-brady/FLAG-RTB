...
typedef struct FLAGRTBInternalContextStruct {
  ...
  // Whether flagrtbSetHostInputBuffer has been called
  bool array_h_set;
  ...
  // Whether flagrtbSetHostOutputBuffer has been called
  bool matrix_h_set;
  bool register_host_matrix;
} FLAGRTBInternalContext;
...
static FLAGRTBInfo compiletime_info = {
  .npol =        NPOL,
  .nstation =    NSTATION,
  .nbaseline =   NBASELINE,
...
#ifdef FIXED_POINT
  .input_type =  FLAGRTB_INT8,
#else
  .input_type =  FLAGRTB_FLOAT32,
#endif
...
static const char flagrtb_version[] = STRINGIFY(FLAGRTB_VERSION);

const char * flagrtbVersionString()
{
  return flagrtb_version;
}

// Populate FLAGRTBInfo structure with compile-time parameters.
...
// Initialize the FLAGRTB.  The device number is intentionally not part of the
...
{
  int error = FLAGRTB_OK;

  CUBE_INIT();

  // Allocate internal context
  FLAGRTBInternalContext *internal = (FLAGRTBInternalContext *)malloc(sizeof(FLAGRTBInternalContext));
  if(!internal) {
    // Uh-oh!
    return FLAGRTB_OUT_OF_MEMORY;
  }
  context->internal = internal;
  internal->device = device_flags & FLAGRTB_DEVICE_MASK;
  internal->array_h_set  = false;
  internal->matrix_h_set = false;
  internal->register_host_array  = true;
  internal->register_host_matrix = true;
  if( device_flags & FLAGRTB_DONT_REGISTER_ARRAY ) {
	  internal->register_host_array = false;
  }
  if( device_flags & FLAGRTB_DONT_REGISTER_MATRIX ) {
  ...
  if( internal->register_host_array ) {
	  // TODO error check
	  flagrtbSetHostInputBuffer(context);
  }
  ...
  if( internal->register_host_matrix ) {
	  // TODO error check
	  flagrtbSetHostOutputBuffer(context);
  }
  ...
  error = flagrtbClearDeviceIntegrationBuffer(context);
  if(error != FLAGRTB_OK) {
    return error;
  }
  ...
  if((NFREQUENCY * NSTATION * NPOL > deviceProp.maxTexture2D[0]) ||
    ...
    return FLAGRTB_INSUFFICIENT_TEXTURE_MEMORY;
  }
#elif TEXTURE_DIM == 1
#if 0
...
int flagrtbClearDeviceIntegrationBuffer(FLAGRTBContext *context)
{
  ...
  FLAGRTBInternalContext *internal = (FLAGRTBInternalContext *)context->internal;
  if(!internal) {
    return FLAGRTB_NOT_INITIALIZED;
  }
  //assign the device
  cudaSetDevice(internal->device);
...
int flagrtbCudaXengine(FLAGRTBContext *context, int syncOp)
{
  ...
  // flagrtbSetHostInputBuffer and flagrtbSetHostOutputBuffer must have been called
  if( !internal->array_h_set || !internal->matrix_h_set ) {
    return FLAGRTB_HOST_BUFFER_NOT_SET;
  }
  ...
  // buffer 0.  This is a no-op unless previous call to flagrtbCudaXengine() had
